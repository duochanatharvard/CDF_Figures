function m_grid_latex(varargin);
...
if ~isempty(xtick),
 ...
 for k=ik,
   ...
   text(xx(k),yy(k)-0.07,labs{k},'horizontalalignment',horizk,'verticalalignment',vertk, ...
...
if strcmp(tickstyle,'dm'),
   ...
   for k=1:length(vals),
     if rem(vals(k),1)==0,
       ...
       L{k}=sprintf([' \\textsf{ %3.0f $^\\circ$' labname(nam) ' }'],vals(k));
   ...
   if ~any(fs==1),  
	...
	%s  sprintf([' %2.2f'' '],rem(vals(k),1)*60)};
    L{k}={sprintf([' \\textsf{ %3.0f $^\\circs' labname(nam) ' } '],fix(vals(k))),...
